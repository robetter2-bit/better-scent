            ...
            {pedidos.map(p => (
              <div key={p.id} style={{ background: 'white', padding: '16px', borderRadius: '12px', marginBottom: '8px', border: p.estado === 'pendiente' ? '2px solid #7c3aed' : '1px solid #e5e7eb' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  ...
                  <div style={{ textAlign: 'right' }}>{p.precioEstimado && <p style={{ fontWeight: 'bold', color: '#7c3aed' }}>{$(p.precioEstimado)}</p>}{p.estado === 'pendiente' && <button onClick={() => surtir(p.id)} style={{ marginTop: '4px', padding: '4px 8px', backgrou
